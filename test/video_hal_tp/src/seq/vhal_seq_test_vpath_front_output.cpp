...
class VhalSeqTestVpathFrontOutputApiSpecification : public VhalSeqTest, public ::testing::WithParamInterface<VhalSeqTestVpathCameraParameter> {};

INSTANTIATE_TEST_CASE_P(VhalSeqTestVpathFrontOutput, VhalSeqTestVpathFrontOutputApiSpecification, ::testing::Values(
	VhalSeqTestVpathCameraParameter{SURFACE_ID_FRONT_DTV, VPATH_NAME_DTV}
));
 ...
 * 映像位置、サイズ変更
 */
TEST_P(VhalSeqTestVpathFrontOutputApiSpecification, SetFrontOutput)
{
	...
	on = true;
	EXPECT_EQ(VIDEO_HAL_API_SUCCESS, cockpit::hal::video_hal::SetValue(obj, "vpath.front.control.output.x", &status_x_expect, sizeof(status_x_expect), VIDEO_HAL_DATA_TYPE_NUM));
    EXPECT_EQ(VIDEO_HAL_API_SUCCESS, cockpit::hal::video_hal::SetValue(obj, "vpath.front.control.output.y", &status_y_expect, sizeof(status_y_expect), VIDEO_HAL_DATA_TYPE_NUM));
	EXPECT_EQ(VIDEO_HAL_API_SUCCESS, cockpit::hal::video_hal::SetValue(obj, "vpath.front.control.output.width", &status_width_expect, sizeof(status_width_expect), VIDEO_HAL_DATA_TYPE_NUM));
    EXPECT_EQ(VIDEO_HAL_API_SUCCESS, cockpit::hal::video_hal::SetValue(obj, "vpath.front.control.output.height", &status_height_expect, sizeof(status_height_expect), VIDEO_HAL_DATA_TYPE_NUM));
 ...
 * 映像位置、サイズ変更
 */
TEST(VhalSeqTestVpathFrontOutputRepeat, SetFrontOutput)
{
	std::unique_lock<std::mutex> lock_sync(mtx_sync);
	bool intime{false};
	bool on{};
	...
	for (size_t i = 0; i < TEST_SEQUENCE_REPEAT_COUNT; i++)
	{
		...
		status_x_expect = 0;
		status_y_expect = 0;
		status_width_expect = 10;
		status_height_expect = 10;
		on = true;
		EXPECT_EQ(VIDEO_HAL_API_SUCCESS, cockpit::hal::video_hal::SetValue(obj, "vpath.front.control.output.x", &status_x_expect, sizeof(status_x_expect), VIDEO_HAL_DATA_TYPE_NUM));
		EXPECT_EQ(VIDEO_HAL_API_SUCCESS, cockpit::hal::video_hal::SetValue(obj, "vpath.front.control.output.y", &status_y_expect, sizeof(status_y_expect), VIDEO_HAL_DATA_TYPE_NUM));
		EXPECT_EQ(VIDEO_HAL_API_SUCCESS, cockpit::hal::video_hal::SetValue(obj, "vpath.front.control.output.width", &status_width_expect, sizeof(status_width_expect), VIDEO_HAL_DATA_TYPE_NUM));
