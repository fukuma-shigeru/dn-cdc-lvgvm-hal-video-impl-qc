...
struct VhalSeqTestHeaconCameraParameter {
	int32_t surface_id;
	std::string front_vpath_name;
};

class VhalSeqTestHeaconFrontOutputApiSpecification : public VhalSeqTest, public ::testing::WithParamInterface<VhalSeqTestHeaconCameraParameter> {};

INSTANTIATE_TEST_CASE_P(VhalSeqTestHeaconFrontOutput, VhalSeqTestHeaconFrontOutputApiSpecification, ::testing::Values(
	VhalSeqTestHeaconCameraParameter{SURFACE_ID_FRONT_DTV, VPATH_NAME_DTV}
));
 ...
 * 映像位置、サイズ変更
 */
TEST_P(VhalSeqTestHeaconFrontOutputApiSpecification, SetFrontOutput)
{
	VhalSeqTestHeaconCameraParameter param = GetParam();
	...
	status_x_expect = 0;
//	status_y_expect = 600;
	status_y_expect = prop.screenHeight - 120;	/* BEVstep3 */
	status_width_expect = prop.screenWidth;	/* スクリーンサイズ幅 */
	status_height_expect = 120;
	on = true;
	EXPECT_EQ(VIDEO_HAL_API_SUCCESS, cockpit::hal::video_hal::SetValue(obj, "heacon.front.control.output.x", &status_x_expect, sizeof(status_x_expect), VIDEO_HAL_DATA_TYPE_NUM));
    EXPECT_EQ(VIDEO_HAL_API_SUCCESS, cockpit::hal::video_hal::SetValue(obj, "heacon.front.control.output.y", &status_y_expect, sizeof(status_y_expect), VIDEO_HAL_DATA_TYPE_NUM));
	EXPECT_EQ(VIDEO_HAL_API_SUCCESS, cockpit::hal::video_hal::SetValue(obj, "heacon.front.control.output.width", &status_width_expect, sizeof(status_width_expect), VIDEO_HAL_DATA_TYPE_NUM));
    EXPECT_EQ(VIDEO_HAL_API_SUCCESS, cockpit::hal::video_hal::SetValue(obj, "heacon.front.control.output.height", &status_height_expect, sizeof(status_height_expect), VIDEO_HAL_DATA_TYPE_NUM));
 ...
 * 映像位置、サイズ変更
 */
TEST(VhalSeqTestHeaconFrontOutputRepeat, SetFrontOutput)
{
	std::unique_lock<std::mutex> lock_sync(mtx_sync);
	bool intime{false};
	bool on{};
...
//		status_y_expect = 600;
		...
		/* もとに戻す */
		status_x = status_x_expect;
		status_y = status_y_expect;
		status_width = status_width_expect;
		status_height = status_height_expect;
		status_x_expect = 0;
		status_y_expect = 0;
		status_width_expect = 0;
		status_height_expect = 0;
		on = true;
		EXPECT_EQ(VIDEO_HAL_API_SUCCESS, cockpit::hal::video_hal::SetValue(obj, "heacon.front.control.output.x", &status_x_expect, sizeof(status_x_expect), VIDEO_HAL_DATA_TYPE_NUM));
		EXPECT_EQ(VIDEO_HAL_API_SUCCESS, cockpit::hal::video_hal::SetValue(obj, "heacon.front.control.output.y", &status_y_expect, sizeof(status_y_expect), VIDEO_HAL_DATA_TYPE_NUM));
		EXPECT_EQ(VIDEO_HAL_API_SUCCESS, cockpit::hal::video_hal::SetValue(obj, "heacon.front.control.output.width", &status_width_expect, sizeof(status_width_expect), VIDEO_HAL_DATA_TYPE_NUM));
